...
//        st.loadData();
    ...
    public void done() throws SQLException {
        selected = tableTransaction.getSelectionModel().getSelectedItem();
        String sql = String.format("update transaction set status = 'done' where transactionID = %s;",selected.getId());
        MainController.stm.executeUpdate(sql);
        load_table();
    }

    public void gotoTransactionAdd(ActionEvent event) throws IOException {

        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("TransactionAdd.fxml"));
        Parent root = loader.load();

        TransactionAddController controller = loader.getController();
